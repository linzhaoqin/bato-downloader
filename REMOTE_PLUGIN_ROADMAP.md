...
- 当前发布版本 `v1.3.5` 的二进制与配置格式不可破坏；任何远端插件配置需向下兼容既有 `config.py` 与 `settings.json`。
- 应用在 macOS / Windows 均以离线模式交付，因此远端插件能力必须提供显式 opt-in 与回退路径，以免影响 `v1.3.5` 用户体验。
...
| - [ ] v0.2 安全+UX | 需求冻结 | 2025-12-12 | Metadata/Checksum、白名单、Beta Flag | v0.1 ✔ |
...
#### 前置条件（v0.1）
- `v1.3.5` 代码基必须保持可构建，且 `PluginManager` 仍由 Tkinter UI 初始化。
...
> ⚠️ 当前 `v1.3.5` GUI 基于 Tkinter，上述 PyQt6 示例仅用于描述交互流程。正式实现时需要在 `ui/tabs/settings_tab.py` 里以 Tk 组件（`ttk.Frame`, `tk.simpledialog` 等）重建同样的 UX，或封装成独立 Tk 窗口。
...
#### 迭代任务清单（v0.2）
- [ ] Registry schema v2 migration/数据迁移脚本，确保老用户升级体验顺滑。
- [ ] Metadata parser + checksum 计算模块上线，并复用至 `scripts/generate_index.py`。
- [ ] Settings UI 新增 metadata 预览弹窗（Tk 版本）与白名单设置。
- [ ] `PLUGIN_REPOSITORY_STRUCTURE` Phase 2：示例插件填写 metadata + 自动校验。
...
- 文档提供发布检查清单，指导如何在 `v1.3.5` 二进制中开启远端更新功能。
...
> 建议在每个阶段结束时发布一个带 Beta Feature Flag 的 `v1.3.5+x` 小版本，方便回收用户反馈并避免一次性堆积风险。
